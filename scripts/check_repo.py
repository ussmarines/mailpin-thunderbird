...
check(manifest.get("browser_specific_settings", {}).get("gecko", {}).get("id") == "ussmarines.mailpin@addons.thunderbird.net", "ID public MailPin modifié")
...
check(package.get("name") == "mailpin-thunderbird", "nom package MailPin inattendu")
...
check(fr_locale.get("brandSubtitle", {}).get("message") == "Suivi d’e-mails & productivité pour Thunderbird.", "sous-titre FR MailPin incohérent")
check(en_locale.get("brandSubtitle", {}).get("message") == "Email Follow-up & Productivity for Thunderbird", "sous-titre EN MailPin incohérent")
check(en_locale.get("brandSlogan", {}).get("message") == "Stay on top. Follow through. Get results.", "slogan MailPin incohérent")
