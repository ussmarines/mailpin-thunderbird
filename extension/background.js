...
const DEFAULT_SETTINGS = Object.freeze({notifications: true, defaultView: "active", openMessageIn: "tab"});
...
function normalizePin(raw, fallbackId = "") {
  ...
  if (!id) return null;
  const checklist = Array.isArray(pin.checklist) ? pin.checklist.slice(0, 100).map((item, index) => ({
...
function normalizeState(raw) {
  ...
  return {
    schemaVersion: 2,
    pins,
    savedViews: Array.isArray(state.savedViews) ? state.savedViews.slice(0, 50).map(view => ({
      id: cleanText(view?.id, 80), name: cleanText(view?.name, 120), query: cleanText(view?.query, 500)
    ...
    history: Array.isArray(state.history) ? state.history.slice(-MAX_HISTORY).map(item => ({
      at: iso(item?.at) || iso(), type: cleanText(item?.type, 40), pinId: cleanText(item?.pinId, 80), detail: cleanText(item?.detail, 200)
    })) : []
  };
}
...
async function saveState(state) { await messenger.storage.local.set({[STATE_KEY]: normalizeState(state)}); }
...
function record(state, type, pinId, detail = "") {
  state.history.push({at: iso(), type: cleanText(type, 40), pinId: cleanText(pinId, 80), detail: cleanText(detail, 200)});
...
function pinFromMessage(message, preset = "simple") {
  ...
  if (preset === "today") {
    due.setHours(23, 59, 59, 999); pin.dueAt = iso(due);
  } else if (preset === "tomorrow") {
    due.setDate(due.getDate() + 1); due.setHours(23, 59, 59, 999); pin.dueAt = iso(due);
  } else if (preset === "waiting") {
    pin.status = "waiting";
  } else if (preset === "noReply") {
    pin.status = "waiting"; pin.waitingForReply = true;
  }
  return pin;
}
...
async function pinMessages(messages, preset = "simple") {
  ...
  const result = await mutateState(state => {
    ...
    for (const message of messages.slice(0, MAX_LIST)) {
      ...
      state.pins[fresh.id] = old ? {...fresh, ...old, lastKnownMessageId: fresh.lastKnownMessageId, folderId: fresh.folderId || old.folderId, accountId: fresh.accountId || old.accountId, subject: fresh.subject || old.subject, author: fresh.author || old.author, nativeTags: fresh.nativeTags, updatedAt: iso()} : fresh;
...
async function unpinIds(ids) {
  const result = await mutateState(state => {
    ...
    for (const id of cleanList(ids, MAX_LIST, 80)) {
      ...
      delete state.pins[id]; record(state, "unpin", id); changed += 1;
    }
    return {changed};
  });
  await rescheduleAlarms();
...
async function resolveMessage(pin) {
  ...
  if (pin.folderId) {
    const scoped = await collect(await messenger.messages.query({headerMessageId: pin.headerMessageId, folderId: pin.folderId}));
...
async function listPins() {
  ...
  for (const pin of Object.values(state.pins).slice(0, 1000)) {
    try {
      ...
      pins.push(message ? {...pin, missing: false, lastKnownMessageId: message.id, folderId: message.folder?.id || pin.folderId, accountId: message.folder?.accountId || pin.accountId, subject: cleanText(message.subject, 1000) || pin.subject, author: cleanText(message.author, 1000) || pin.author, nativeTags: cleanList(message.tags, 100, 100)} : {...pin, missing: true});
    } catch {
      pins.push({...pin, missing: true});
    }
  }
  return {pins, settings: state.settings, savedViews: state.savedViews, history: state.history.slice(-50)};
}
...
async function updatePin(id, patch) {
  return mutateState(state => {
    const key = cleanText(id, 80); const pin = state.pins[key];
...
async function openPin(id) {
  const state = await loadState(); const pin = state.pins[cleanText(id, 80)];
  if (!pin) throw new Error("Pin not found");
  const message = await resolveMessage(pin); if (!message) throw new Error("Message not found");
  return messenger.messageDisplay.open({messageId: message.id, location: state.settings.openMessageIn === "window" ? "window" : "tab", active: true});
}
async function actOnPin(id, action) {
  ...
  if (action === "archive") { await messenger.messages.archive([message.id]); return {ok: true}; }
  if (action === "trash") { await messenger.messages.delete([message.id], {deletePermanently: false, isUserAction: true}); return {ok: true}; }
  throw new Error("Unsupported action");
}
async function listNativeTags() { return messenger.messages.tags.list(); }
async function setNativeTags(id, tags) {
  ...
  return mutateState(next => { if (next.pins[id]) { next.pins[id].nativeTags = selected; next.pins[id].updatedAt = iso(); record(next, "tags", id); } return selected; });
}

async function rescheduleAlarms() {
  await messenger.alarms.clearAll();
  const state = await loadState(); const now = Date.now();
  for (const pin of Object.values(state.pins)) {
    if (!pin.dueAt || pin.status === "completed") continue;
    const when = new Date(pin.dueAt).getTime();
    if (Number.isFinite(when)) messenger.alarms.create(`pin:${pin.id}`, {when: Math.max(when, now + 1000)});
  }
}
async function handleAlarm(alarm) {
  if (!alarm?.name?.startsWith("pin:")) return;
  const id = alarm.name.slice(4); const state = await loadState(); const pin = state.pins[id];
  if (!pin || pin.status === "completed" || !state.settings.notifications) return;
  await messenger.notifications.create(`mailpin:${id}`, {type: "basic", iconUrl: messenger.runtime.getURL("icons/mailpin-icon.svg"), title: "MailPin — rappel", message: pin.subject || "Un e-mail suivi arrive à échéance."});
}
const openDashboard = () => messenger.tabs.create({url: messenger.runtime.getURL("dashboard/dashboard.html")});

const MENU = Object.freeze({toggle: "mailpin-toggle", today: "mailpin-today", tomorrow: "mailpin-tomorrow", waiting: "mailpin-waiting", noReply: "mailpin-no-reply", dashboard: "mailpin-dashboard"});
async function createMenus() {
  await messenger.menus.removeAll();
  messenger.menus.create({id: MENU.toggle, title: "MailPin — Épingler / désépingler", contexts: ["message_list"]});
  messenger.menus.create({id: MENU.today, title: "MailPin — À traiter aujourd'hui", contexts: ["message_list"]});
  messenger.menus.create({id: MENU.tomorrow, title: "MailPin — À traiter demain", contexts: ["message_list"]});
  messenger.menus.create({id: MENU.waiting, title: "MailPin — En attente", contexts: ["message_list"]});
  messenger.menus.create({id: MENU.noReply, title: "MailPin — Attendre une réponse", contexts: ["message_list"]});
  messenger.menus.create({id: MENU.dashboard, title: "Tableau de bord MailPin", contexts: ["tools_menu"]});
}
messenger.menus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === MENU.dashboard) return openDashboard();
  const messages = await collect(info.selectedMessages || (tab?.id ? await messenger.mailTabs.getSelectedMessages(tab.id) : null));
...
messenger.commands.onCommand.addListener(async (command, tab) => {
  ...
  const messages = await selectedMessages(tab.id);
  ...
  const state = await loadState();
  const ids = messages.map(message => stablePinId(message.headerMessageId, message.id)).filter(id => state.pins[id]);
...
messenger.messageDisplayAction.onClicked.addListener(async tab => toggleMessages(await displayedMessages(tab.id)));
...
messenger.notifications.onClicked.addListener(async notificationId => {
  if (!notificationId.startsWith("mailpin:")) return;
  await openPin(notificationId.slice(8)).catch(() => openDashboard());
});
...
messenger.runtime.onMessage.addListener(async request => {
  ...
  if (type === "mailpin:update") { const result = await updatePin(request.id, request.patch); await rescheduleAlarms(); return result; }
  ...
  if (type === "mailpin:import") {
    ...
    await mutateState(state => { Object.assign(state, incoming); record(state, "import", "system"); });
    await rescheduleAlarms(); return {ok: true};
  }
...
messenger.runtime.onInstalled.addListener(async () => { await createMenus(); await rescheduleAlarms(); });
