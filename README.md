...
1. Téléchargez `MailPerch_v1.1.2.xpi` depuis la release `v1.1.2`.
...
> Les anciennes builds internes `3.2.x` utilisaient un numéro supérieur aux versions publiques actuelles. Pour tester `1.1.2` après une build de développement, utilisez un profil de test propre ou désinstallez d’abord l’ancienne build après avoir exporté vos données MailPerch.
...
- [Dernier audit de sécurité complet : 1.1.1](SECURITY_AUDIT_1.1.1.md)
- [Dernier rapport de validation Thunderbird : 1.1.1](VALIDATION_REPORT_1.1.1.md)
...
MailPerch est distribué sous la **MailPerch Source-Available License 1.0**. La consultation par les reviewers Mozilla/Thunderbird et la distribution officielle par le propriétaire sont autorisées ; la redistribution publique, les forks distribués et l’exploitation commerciale nécessitent une autorisation écrite. Consultez [LICENSE](LICENSE).
