...
| ID | Introduit | Symptôme | Cause | Fichiers | Test | Statut | Correction | Validation |
|---|---|---|---|---|---|---|---|---|
| MP-2026-061 | 1.7.5 | Après un redémarrage complet de Thunderbird, les épingles persistées ne réapparaissaient dans la boîte mail qu’après une action MailPin telle que l’ouverture du Dashboard. | Le background Manifest V3 ne s’enregistrait pas sur `runtime.onStartup` : il restait arrêté au cold start, puis le clic sur l’action MailPin le réveillait et le `mailTabs.query({})` top-level injectait enfin le panneau. Le banc persistant masquait le défaut en réactivant artificiellement l’onglet mail. | `extension/background.js`, banc Thunderbird persistant | cold start réel sur deux processus + smoke Thunderbird 154.0 | À VALIDER | 1.7.6 | Correctif runtime PR #64 head `26fc0ac9b4d35009f125f543eefc5de9338bef71` : QA `32639780333` PASS, smoke Thunderbird 154.0 `32639780313` PASS ; baseline background `stopped`, panneau/toggle `0`, puis correction `APP_STARTUP`, 50 épingles SQLite préservées, 34 cartes A+C, panneau/toggle `1`, zéro timeout/exception. Fusion `main` `fa6782f8ecfaf259d9b8e54a08e5cf361172c669`; candidate versionnée 1.7.6 encore à valider avant publication. |
| MP-2026-059 | 1.7.3 | Thunderbird 154 refuse MailPin comme incompatible avant le démarrage de l’extension. | Le manifeste et les gardes de compatibilité limitaient `strict_max_version` à `153.*`; le smoke runtime était lui aussi épinglé sur Thunderbird 153.0.1 ESR. | `extension/manifest.json`, `.github/workflows/thunderbird-smoke.yml`, gardes et documentation de compatibilité | QA + smoke réel Thunderbird 154.0 | CORRIGÉ | 1.7.4 | Candidate exacte `c2527b57de4775f4fd228af22b9792937e7ce6ea` : QA `32300356172` PASS et smoke Thunderbird 154 `32300356085` PASS ; tag `v1.7.4` identique à `b74c0c7f264cf387269be0aaf18e47e99cf07600`. |
| MP-2026-060 | 1.7.4 | ATN refuse la soumission car le nom embarqué de l’extension dépasse 50 caractères. | `extensionName` vaut `MailPin — Email Follow-up & Productivity for Thunderbird` (56 caractères) dans les locales FR/EN. | `extension/_locales/*/messages.json`, métadonnées et gardes de nom | `scripts/check_repo.py`, `tests/static_checks.py`, QA/build/smoke | CORRIGÉ | 1.7.5 | Nom `MailPin — Email Follow-up & Productivity` = 40 caractères ; candidate `19cf23c21e983be924ffd9e6af8fdb1e8e612947` : QA `32480175617` PASS et smoke Thunderbird 154 `32480175435` PASS ; tag `v1.7.5` `2384ee52df95a711424dfeb817ef114888634ed0` ; artefacts publics `32481646372` PASS. |
