...
Aucun bug ouvert connu dans le code de la build 3.2.5 au moment de sa création.
Les constats découverts après installation doivent être ajoutés ici sans supprimer
les entrées historiques.
...
| MP-2026-004 | 3.2.4 | Deux étoiles apparaissent ou se chevauchent dans les cartes de messages. | Les annotations et le repositionnement du mode `nativeStar` restaient appliqués en mode indépendant sur les lignes virtualisées. | `extension/api/pinInbox/implementation.js`, `extension/styles/pin.css` | `tests/test_regressions_3_2_5.py` | À VALIDER | 3.2.5 | À tester dans Thunderbird 153 en mode indépendant et `nativeStar`. |
| MP-2026-005 | 3.2.4 | Les boutons Enregistrer et Annuler des paramètres semblent inopérants. | Les actions dépendaient uniquement de gestionnaires de clic hors formulaire et mutaient directement l’objet retourné par l’API. | `extension/options/options.html`, `extension/options/options.js` | `tests/test_regressions_3_2_5.py` | À VALIDER | 3.2.5 | À tester dans Thunderbird après modification de chaque type de champ. |
