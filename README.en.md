...
> **Status: development build 3.2.5.** Static checks, model tests and regression guards run automatically. A complete real-Thunderbird GUI validation is still required before release.
...
Build with `npm run build`, then install `dist/MailPerch_v3.2.5.xpi` from Thunderbird’s Add-ons Manager. Restart Thunderbird completely and use a test profile for destructive actions.
